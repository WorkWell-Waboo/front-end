					...
					<Carousel setApi={setCarouselBlog} opts={{align: "start", loop: true}}> 
						<CarouselContent >
							{Array.from({ length: 5 }).map((_, index) => (
								<CarouselItem className=" lg:basis-1/2"  key={`carousel-item-${Date.now()}-${index}`}>
									<WBox 
										variant="white"
										id={index}
										imageSrc={PhotoBlog}
			...
			<div className="block">
				...
				<div className="relative w-full">
					<button 
						type="button"
						className="absolute -left-2 top-1/2 transform -translate-y-1/2 z-2 bg-[#F9F3FF] w-7 h-7 flex justify-center items-center drop-shadow-md rounded-full cursor-pointer" 
						onClick={()=> carouselWebnar?.scrollTo(currentCarouselWebnar - 1)}
					>
						<Arrow className="text-primary w-1.5 h-2"/>
					</button>
					<button 
					...
					<Carousel setApi={setCarouselWebnar} opts={{align: "start", loop: true}}> 
						<CarouselContent >
							{Array.from({ length: 5 }).map((_, index) => (
								<CarouselItem className=" lg:basis-1/2"  key={`carousel-item-${Date.now()}-${index}`}>
									<WBox 
